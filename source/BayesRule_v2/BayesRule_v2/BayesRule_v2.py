...
class BayesRule:
    def __init__(self):
        ...
        self.EstimateTools = False
        self.wasEstimateTools = False
        self.without_log = False
        self.LaplaceSmoothing = True
...
######################################################################
    ...
    def likelihood_prob(self, material, category):
        ...
        if self.EstimateTools:
            ...
            # denominator_count = sum(self.material_tool_count[category].values()) + len(self.MaterialNames) # naive bayes
            denominator_count = self.tool_count[category]
            print 'dim_count = %d' % numerator
            print 'denominator_count = %d' % denominator_count
            ...
            numerator = self.num_of_appearance(material, category) + laplaceSmoothing # + 1 is Laplace Smoothing
            # print 'count(%s , %s) = %d' % (material, category, numerator)
            # denominator_count = sum(self.material_motion_count[category].values()) + len(self.MaterialNames) # naive bayes
            denominator_count = self.motion_count[category]
            # print 'denominator_count = %s' % denominator_count
            prob = float(numerator) / denominator_count
    ...
    def prior_prob(self, category):
        if self.EstimateTools:
            ...
            #print "p(%s) = %d / %d = %g" % (category, num_of_material_of_the_tool, num_of_tools, float(num_of_material_of_the_tool) / num_of_tools)
...
#        prob = score - math.log(self.multiplication(denominator_list))
        prob = score - math.log(self.denominator)
    ...
    def score_without_log(self, number, materials, category):
        ...
        if self.wasEstimateTools:
            ...
            denominator_list.append(self.denominator_prob(self.estimate_tool[number]))
        # print 'p(%s) : score = %g' % (category, score)
        # print 'score = %g' % score
#        prob = score / self.multiplication(denominator_list)
    ...
    def classify(self):
        ...
        fileName = ""
        if self.EstimateTools:
            fileName = resultpath + 'trial' + str(self.TrialNum) + '\\' + 'Result_ExcludeTool_v2.txt'
        else:
            if self.wasEstimateTools:
                fileName = resultpath + 'trial' + str(self.TrialNum) + '\\' + 'Result_ExcludeMotion_with_tool_v2.txt'
            else:
        ...
        for num in range(len(test_sentence_num)):
            ...
            if self.EstimateTools:
                ...
                print 'best tool is %s\n' % best_guessed_target
                # write db #
                WriteStatisticsSheet.write(write_row + num, 5, best_guessed_target)

                self.estimate_tool[test_sentence_num[num]] = best_guessed_target
            else:
                self.denominator = self.MarginalProbability(Materials, test_sentence_num[num])
                if self.denominator == 0.0:
                    best_guessed_target = "None"
                ...
                # write db #
                if self.wasEstimateTools:
                    WriteStatisticsSheet.write(write_row + num, 6, best_guessed_target)
                else:
                    WriteStatisticsSheet.write(write_row + num, 4, best_guessed_target)
...
if __name__ == "__main__":
    ...
    if br.exitDB(SentenceDBFile):
        ...
        for trial in range(1):#(br.TrialNum):
            br.roadTestData()
            if br.EstimateTools:
                print "material -> tool"
                br.train()
